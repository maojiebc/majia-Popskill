...
**curation 优先于 quantity（2026-05-15 沉淀）**：

dotey 长推（2026-05-11，126K views）下 @lupinlin 评论指出："*MCP 标准化 + Skill 分发这个方向确实是对的，但目前的挑战是 Skill 质量参差不齐。类比 npm 生态的早期——注册容易但维护难。关键看 OpenAI 怎么做 curation 和 trust 机制。*"

popskill 的回应是**两层一起跑**：

- **AgentShield**（automated trust gate）：解决"不带恶意代码"——自动化、可扩展、覆盖全部 install 路径
- **Editor's Pick**（curation signal）：解决"真的好用"——人工策展、有立场、只放头部 Skills 开发者的代表作（见 [docs/editors-pick-seeds.md](../docs/editors-pick-seeds.md)）

popskill 不做"全网 skill 都收"的中性 marketplace。**是有立场的精选**，宁可慢，不收质量低的 skill。这跟 npm/PyPI 的"中性注册中心"是不同的产品定位——更接近 App Store / Setapp 那种 curated 模式。
...
**Plugin namespace 决策（2026-05-15 沉淀，已收敛）**：

**Plugin namespace 是只读旁路，不入 store、不投影、不作为 component kind。**

背景：dotey 2026-05-11 长推（126K views）提出"Agent 版 App Store"概念，特指 Codex / Cursor / Claude Desktop 这些 host app 各自的**插件市场**——比如 Codex Presentations Plugin。同期 majia 与 Codex 在 2026-05-15 thread `019e2aaa` 的本地 skill 治理设计讨论，对这层"插件"的归类给出了明确结论（详见 [docs/architecture-reference-skillctl.md](../docs/architecture-reference-skillctl.md)）。

这层"插件"跟当前 7 类 component 都不太一样：

- 不是 Skill（host 特定格式，不是 SKILL.md）
- 不是 CLI（不在终端跑）
- 不是 MCP（不走 MCP 协议）
- 不是 Agent persona

**已收敛的设计原则**（来自 architecture-reference-skillctl.md 不变量 1）：
...
- ❌ 不新增 `Plugin` 一类 `AssetComponentKind` enum 值（Rust + Swift + tests 不动）
- ✅ `skill-cli doctor --inventory`（v0.1.x 计划新增）扫描时**枚举 plugin namespace 但标 `external`**，让用户知道"这是 plugin 来的，popskill 管不到"
- ✅ deployment 层支持 host plugin 接口作为可选 target——通过 `wrapper` / `configPatch` 策略，**不引入新 component kind**

完整设计输入：[docs/architecture-reference-skillctl.md](../docs/architecture-reference-skillctl.md)（含三层数据模型、5 个硬不变量、publisher/consumer 解耦图）。
...
| P1 | Presentations | baoyu-presentations + Anthropic pptx skill + 模板 | dotey 在 Codex Presentations Plugin 印证赛道（[why-popskill.md §3](../docs/why-popskill.md) 业内当事人证词） |
...
- ✅ `scripts/dev-build.sh`、`scripts/ci-local.sh`、read-only smoke、mutating smoke、bundle/release smoke、development DMG 打包、release manifest/appcast 与严格 release doctor gate 已落地
