...
> 作者：majia
>
> v2 更新：补强了「业内当事人证词」一节，引用了 2026-05-11 dotey（宝玉）那条 126K views 的长推 +
> 2026-05-13 op7418（归藏）的接力推作为头号证据。原"飞书 CLI 破万"作为次级证据保留。
...
讲完我自己的痛点，最有分量的证词来自两位真正在生态里写 skill、被广泛 fork 的头部开发者。

**dotey（宝玉）2026-05-11 那条 126K views 的长推**（[原文](https://x.com/dotey/status/2053940091970580736)，709 赞，2495 字），把 skill 生态的卡点讲得最透：

> "我自己的 baoyu-skills 快 2 万 Star 了，但从中赚到的钱是 $0。Skill 这东西几乎是透明的，对 Agent 透明，对人也透明，复刻成本极低，不管你写得再好，护城河都很浅。"
>
> "MCP 解决了'连接'问题... Agent Skills 解决了'怎么做'的问题... 但有一块缺口始终没补上：用户的二次编辑。"
>
> "最合理的路，是插件机制。把 Agent 做成平台，让社区来贡献插件，就像 VSCode 和 Chrome 那样... **App Store 和 Chrome 插件市场已经跑通了一套收费和版权保护机制，把它移植到 Agent 插件市场完全可行。**"
>
> "这个窗口不会开太久。先进去的能拿到冷启动红利，晚进去的只剩存量竞争。**时间点不会太远，也许就在这几个月。**"

这条推下面 124 条评论里有几条更狠：

> [@lupinlin] "MCP 标准化+Skill 分发这个方向确实是对的，但目前的挑战是 Skill 质量参差不齐。**类比 npm 生态的早期——注册容易但维护难。关键看 OpenAI 怎么做 curation 和 trust 机制。**"
>
> [@lilililiMozi] "**Agent 的插件就是下个时代的 App Store。**"

**op7418（归藏）2026-05-13 接力**（[原文](https://x.com/op7418/status/2054414353731109224)，25K views）把"头部 Skills 开发者"群体名挑明：

> "像藏师傅、宝玉、乔木、一泽这种头部的 Skills 开发者。他们的 Skills 装机量其实非常大，遍布各种 Agent。无论是从数据、用户反馈，还是从打广告这种商业价值来说，潜力都非常大。
>
> 目前大家都没有把这条线重视起来，没有一个合适的合作方式给这些开源 Skills 开发者提供好的框架或商业化路径..."

他在自己评论 #2 给出的答案：

> "**我们需要一个聚焦的公共空间，去分发、展示、曝光和连接。还有就是服务好开发者尤其是优质开发者。**"

op7418 这条评论的最后一句，几乎是 popskill 的产品定位逐字稿。

把这些放一起看——**头部 Skills 开发者已经在公开喊"需要一个跨 Agent 的、能服务开发者的、能解决商业化的公共空间"**。他们喊出来的东西不一定是 popskill 这个形态，但 popskill 押的就是这个赛道，押的依据就是这群人的真实痛点。

---

## 四、三个数字证据：这不是少数人的怪癖，是大势所趋
...
## 六、popskill 的回答：asset control plane
...
它的底层判断是：**本地 AI 能力正在膨胀成一类基础设施，需要一个统一的 asset control plane 来管。** 头部 Skills 开发者 dotey 自陈"baoyu-skills 快 2 万 Star，从中赚到的钱是 \$0"——这是 Skills 商业化困境最权威的当事人证词。同期 op7418 公开喊"我们需要一个聚焦的公共空间，去分发、展示、曝光和连接"。飞书 CLI 破万星证明了 capability package 已成主流，CC Switch 6.8 万星证明了用户对"集中管理"是有需求的，Anthropic MCP Registry 立项证明了上游基础设施在搭，但没人站在「Mac 原生 + 5 类 component + App Store UX + 使用统计 + 服务开发者」的交汇处。
