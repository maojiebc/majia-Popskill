...
const psCardStyles = {
  ...
  pill: (status) => {
    const map = {
      on:     { bg: "rgba(26,154,78,0.07)", border: "#a5d1b3", c: "#177a3e" },
  ...
  kbFocusRing: { borderColor: "#1f4ed8", boxShadow: "0 0 0 2px rgba(31,78,216,0.18)" },
  kbPillRing: { borderRadius: 999, boxShadow: "0 0 0 2px rgba(31,78,216,0.40)" },
  ...
  aggBlock: { display: "flex", flexDirection: "column", alignItems: "center", gap: 3, width: 52, flexShrink: 0 },
  headVer: { width: 96, textAlign: "right", fontSize: 11.5, color: "#7c7869", fontVariantNumeric: "tabular-nums", whiteSpace: "nowrap", flexShrink: 0 },
  headActs: { width: 46, display: "flex", gap: 2, justifyContent: "flex-end", flexShrink: 0 },
  ...
  colCell: { width: 52, display: "flex", justifyContent: "center", flexShrink: 0 },
  colVer: { width: 96, textAlign: "right", fontSize: 11, color: "#9a9684", fontVariantNumeric: "tabular-nums", flexShrink: 0 },
  colAct: { width: 46, flexShrink: 0, textAlign: "right" },
};
...
// ── 详情 peek（点击能力名称）────────────────────────
function PsDetailPeek({ peek, onOpenEditor, onClose }) {
  const { cap, entry } = peek;
  const W = 380;
  const left = Math.max(12, Math.min(peek.x - W / 2, 1280 - W - 12));
  const readme = psReadme(cap.id);
  const fromBundle = entry.id !== cap.id;
  const statRow = (tool, label) => {
    const s = cap[tool];
    const m = { on: ["●", "已激活", "#1a9a4e"], off: ["—", "未链接", "#9a9684"], stub: ["◐", "占位待校验", "#b88300"], broken: ["✕", cap.brokenCause || "断链", "#c01818"] }[s] || ["—", s, "#9a9684"];
    return (
      <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 11.5, fontWeight: 600, color: m[2], whiteSpace: "nowrap" }}>
        <span style={{ ...psStyles.mono, fontSize: 12 }}>{m[0]}</span>{label} <span style={{ fontWeight: 500, opacity: 0.85 }}>{m[1]}</span>
      </span>
    );
  };
  return (
    <React.Fragment>
      <div style={{ position: "absolute", inset: 0, zIndex: 40 }} onClick={onClose}></div>
      <div data-screen-label="详情 peek" style={{
        position: "absolute", zIndex: 41, width: W,
        left, top: peek.flip ? undefined : peek.y + 6, bottom: peek.flip ? 820 - peek.y + 28 : undefined,
        background: "#fff", borderRadius: 9, border: "1px solid #dcd9cf",
        boxShadow: "0 12px 36px rgba(0,0,0,0.18)", overflow: "hidden",
        fontFamily: psStyles.win.fontFamily,
      }}>
        <div style={{ padding: "11px 14px 9px", borderBottom: "1px solid #efede5", background: "#fafaf8" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
            <span style={{ fontSize: 13.5, fontWeight: 700, letterSpacing: "-0.015em", color: "#111", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{cap.name}</span>
            <span style={psStyles.typeTag(cap.type)}>{cap.type}</span>
            <span style={psStyles.spacer}></span>
            <span style={{ ...psStyles.kbd, fontSize: 10, padding: "1px 6px", cursor: "pointer", flexShrink: 0 }} onClick={onClose}>esc</span>
          </div>
          <div style={{ fontSize: 11, color: "#9a9684", marginTop: 3, fontVariantNumeric: "tabular-nums" }}>
            v{cap.version} · {cap.author}{cap.tokens > 0 ? <span> · {(cap.tokens / 1000).toFixed(1)}k tokens</span> : null}
            {fromBundle && <span style={{ ...psStyles.mono, fontSize: 10 }}> · ⊂ {entry.name}</span>}
          </div>
        </div>
        <div style={{ padding: "11px 14px" }}>
          <div style={{ fontSize: 12, color: "#444", lineHeight: 1.55 }}>{cap.desc}</div>
          {readme && (
            <div style={{ marginTop: 9 }}>
              <div style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", color: "#b3ae9e", marginBottom: 4 }}>SKILL.md · 文档摘要</div>
              <div style={{ background: "#fcfbf6", border: "1px solid #efede5", borderRadius: 6, padding: "8px 10px", fontSize: 11.5, color: "#5e5a4e", lineHeight: 1.6 }}>{readme}</div>
            </div>
          )}
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: 10 }}>
            {statRow("claude", "Claude")}
            {statRow("codex", "Codex")}
          </div>
          {entry.sourceUrl && (
            <div style={{ ...psStyles.mono, fontSize: 10.5, color: "#7c7869", marginTop: 8, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>↗ {entry.sourceUrl}</div>
          )}
        </div>
        <div style={{ padding: "9px 14px", borderTop: "1px solid #efede5", background: "#fafaf8", display: "flex", alignItems: "center" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 6, height: 26, padding: "0 11px", borderRadius: 6, border: "1px solid #d8d5cb", fontSize: 11.5, fontWeight: 600, color: "#444", cursor: "pointer", whiteSpace: "nowrap", background: "#fff" }}
            onClick={() => { onOpenEditor(cap.name); onClose(); }}>↗ 在编辑器中打开</div>
          <span style={psStyles.spacer}></span>
          <span style={{ fontSize: 10.5, color: "#b3ae9e" }}>完整文档在 SKILL.md</span>
        </div>
      </div>
    </React.Fragment>
  );
...
function PsMain({ db, winRef, flashId, expandAll, onToggle, onResolve, onApplyUpdate, onFixAll, onUpdateAll, onOpenAdd, onRemoveEntry, onOpenEditor, onScanLocal }) {
  ...
  const [focusTool, setFocusTool] = useState("claude");
  ...
  useEffect(() => {
    const h = (e) => {
      ...
      if (e.key === "Escape") { setPop(null); setPeek(null); setFocusIdx(-1); }
      if (/INPUT|TEXTAREA/.test(document.activeElement.tagName)) return;
      const kb = kbRef.current;
      if (kb.pop || kb.peek) return;
      if (e.key === "ArrowDown" || e.key === "ArrowUp") {
        e.preventDefault();
        const d = e.key === "ArrowDown" ? 1 : -1;
        setFocusIdx(i => i < 0 ? (d > 0 ? 0 : kb.focusList.length - 1) : Math.max(0, Math.min(kb.focusList.length - 1, i + d)));
      } else if ((e.key === "ArrowLeft" || e.key === "ArrowRight") && kb.focusIdx >= 0) {
        e.preventDefault();
        setFocusTool(e.key === "ArrowLeft" ? "claude" : "codex");
      } else if ((e.key === " " || e.key === "Enter") && kb.focusIdx >= 0) {
        e.preventDefault();
        const it = kb.focusList[kb.focusIdx];
        if (!it) return;
        if (it.t === "bundle") { if (!kb.q) toggleExpand(it.id); }
        else {
          const el = gridRef.current && gridRef.current.querySelector(`[data-kbcell="${it.id}:${kb.focusTool}"]`);
          const tgt = el && (el.firstElementChild || el);
          if (tgt) tgt.click();
        }
      }
    };
  ...
  const toDesign = (e) => {
    ...
    return { x: (r.left + r.width / 2 - wr.left) / k, y: Math.min((r.bottom - wr.top) / k, 800) };
  };
  ...
  // 名称点击 → 详情 peek
  const onNameClick = (e, cap, entry) => {
    e.stopPropagation();
    const { x, y } = toDesign(e);
    setPop(null);
    setPeek({ cap, entry, x, y, flip: y > 430 });
  };
  ...
  // 键盘导航：焦点序列与可见顺序一致；当前帧状态存入 ref 供事件处理器读取
  const focusList = [];
  items.forEach(it => {
    if (it.kind === "bundle") {
      focusList.push({ t: "bundle", id: it.entry.id });
      (it.kids || []).forEach(c => focusList.push({ t: "cap", id: c.id }));
    } else focusList.push({ t: "cap", id: it.cap.id });
  });
  kbRef.current = { focusList, q, focusIdx, focusTool, pop, peek };
  const focusedId = focusIdx >= 0 && focusList[focusIdx] ? focusList[focusIdx].id : null;

  useEffect(() => {
    setFocusIdx(i => i >= 0 && i >= focusList.length ? focusList.length - 1 : i);
  }, [focusList.length]);

  // 焦点随动滚动（手动调 scrollTop，换算缩放系数）
  useEffect(() => {
    if (focusIdx < 0 || !gridRef.current) return;
    const it = kbRef.current.focusList[focusIdx];
    if (!it) return;
    const sc = gridRef.current;
    const el = sc.querySelector(`[data-kbrow="${it.id}"]`);
    if (!el) return;
    const k = winRef.current ? winRef.current.getBoundingClientRect().width / 1280 : 1;
    const r = el.getBoundingClientRect(), cr = sc.getBoundingClientRect();
    if (r.top < cr.top + 8) sc.scrollTop += (r.top - cr.top - 8) / k;
    else if (r.bottom > cr.bottom - 8) sc.scrollTop += (r.bottom - cr.bottom + 8) / k;
  }, [focusIdx]);
  ...
  const renderCapCard = (it) => {
    ...
    return (
      <div key={cap.id} data-kbrow={cap.id} style={{ ...psCardStyles.card, ...(flashId === cap.id ? psCardStyles.cardFlash : {}), ...(focusedId === cap.id ? psCardStyles.kbFocusRing : {}) }}
        ...
        <div style={psCardStyles.body}>
          <div style={psCardStyles.nameRow}>
            <span style={{ ...psCardStyles.name, cursor: "pointer" }} title="查看详情"
              onClick={(e) => onNameClick(e, cap, entry)}
              onMouseEnter={nameHover.onMouseEnter} onMouseLeave={nameHover.onMouseLeave}>{psHighlight(cap.name, q)}</span>
            <span style={psStyles.typeTag(cap.type)}>{cap.type}</span>
        ...
        <div style={psCardStyles.toggleCol}>
          <div data-kbcell={`${cap.id}:claude`} style={focusedId === cap.id && focusTool === "claude" ? psCardStyles.kbPillRing : null}>
            <PsTogglePill status={cap.claude} label="Claude" onClick={(e) => onCellClick(e, cap, "claude", entry)} />
          </div>
          <div data-kbcell={`${cap.id}:codex`} style={focusedId === cap.id && focusTool === "codex" ? psCardStyles.kbPillRing : null}>
  ...
  const renderBundleCard = (it) => {
    ...
    return (
      <div key={e.id} data-kbrow={e.id} style={{ ...psCardStyles.card, ...psCardStyles.bundleCard, ...(flashId === e.id ? psCardStyles.cardFlash : {}), ...(focusedId === e.id ? psCardStyles.kbFocusRing : {}) }}
        ...
        <div style={psCardStyles.bundleHead} onClick={() => !q && toggleExpand(e.id)}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ ...psCardStyles.desc, marginTop: 2 }}>{psHighlight(e.desc, q)} <span style={{ ...psStyles.mono, fontSize: 10, color: "#9a9684" }}>· {(e.tokens / 1000).toFixed(1)}k tokens · ↗ {e.sourceUrl}</span></div>
          </div>
          ...
          <div style={psCardStyles.headVer}>
            v{e.version}
          ...
          <div style={psCardStyles.headActs}>
        ...
        {open && (
          <div style={psCardStyles.childList}>
            ...
            {kids.map(c => {
              ...
              const cf = focusedId === c.id;
              return (
                <div key={c.id} data-kbrow={c.id} style={{ ...psCardStyles.childRow, background: cf ? "#eef3fd" : (ch ? "#f4f2ec" : "transparent") }}
                  ...
                  <div style={{ flex: 1, minWidth: 0, display: "flex", alignItems: "baseline", gap: 8 }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 600, color: "#222", whiteSpace: "nowrap", cursor: "pointer" }} title="查看详情"
                      onClick={(ev) => onNameClick(ev, c, e)}
                      onMouseEnter={nameHover.onMouseEnter} onMouseLeave={nameHover.onMouseLeave}>{psHighlight(c.name, q)}</span>
                    <span style={psStyles.typeTag(c.type)}>{c.type}</span>
                    <span style={{ ...psCardStyles.desc, fontSize: 11, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", flex: 1, minWidth: 0 }}>{psHighlight(c.desc, q)}</span>
                  </div>
                  <span data-kbcell={`${c.id}:claude`} style={{ ...psCardStyles.colCell, ...(cf && focusTool === "claude" ? { background: "rgba(31,78,216,0.12)", borderRadius: 6 } : {}) }}><PsCell s={c.claude} title="点击切换 / 处理" onClick={(ev) => onCellClick(ev, c, "claude", e)} /></span>
                  <span data-kbcell={`${c.id}:codex`} style={{ ...psCardStyles.colCell, ...(cf && focusTool === "codex" ? { background: "rgba(31,78,216,0.12)", borderRadius: 6 } : {}) }}><PsCell s={c.codex} title="点击切换 / 处理" onClick={(ev) => onCellClick(ev, c, "codex", e)} /></span>
    ...
    <main data-screen-label="能力矩阵主屏" style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
      ...
      <div style={psStyles.filterRow}>
        ...
        <div style={{ fontSize: 11.5, color: filtering ? "#1f4ed8" : "#888", fontWeight: filtering ? 600 : 400, whiteSpace: "nowrap" }}>
          {filtering ? `${capCount} 项匹配` : "↑↓ 行 · ←→ 列 · 空格 切换"}
        </div>
      </div>

      <div ref={gridRef} style={{ flex: 1, overflow: "auto", background: "#fafaf8" }}>
        {items.length === 0 ? (
      ...
      {peek && <PsDetailPeek peek={peek} onOpenEditor={onOpenEditor} onClose={() => setPeek(null)} />}
    </main>
  );
}

Object.assign(window, { PsMain, PsFixPopover, PsDetailPeek, PsEmptyPane, psFixOptions, PsTogglePill });
