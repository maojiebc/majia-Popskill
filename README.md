...
[**↓ 下载 Popskill（2.7 MB，已签名 + 已公证）**](https://github.com/maojiebc/majia-Popskill/releases/latest/download/Popskill-2.16.0.dmg)
...
<tr>
<td colspan="2"><img src="docs/screenshots/table.png" alt="账本表格视图 — 套装可展开，状态符号矩阵"></td>
</tr>
<tr>
<td colspan="2"><b>卡片 / 表格双视图</b> — 右上角一键切换：卡片看质感，账本表格看密度（套装可展开、状态符号矩阵、覆盖分数）</td>
</tr>
</table>
...
- **内容哈希更新** — 不依赖版本号，逐成员 SHA-256 比对上游；monorepo 一次 clone 检完全员、只换变化的
- **上游新增一键装** — monorepo 上游新出的技能显示「+N 未装」徽标，点名单、一键装进 store 并挂载（v2.17）
- **更新前自动备份** — 被替换的旧版进回收站（保留 200 份），随时可恢复
...
- **界面双语** — 简体中文 + English 全量本地化，跟随系统语言自动切换（v2.12）
- **卡片 / 表格双视图** — 同一份数据，卡片矩阵看质感、账本表格看密度，右上角一键切换（v2.13）
...
**Q：它会动我的文件吗？**
防呆三条铁律：只删 symlink、真实目录一律进回收站（`~/.agents/.trash/`，留 200 份）、store 目录绝不被开关动到。40 个引擎单测盯着这些行为。
...
当前 [v2.16.0](https://github.com/maojiebc/majia-Popskill/releases/tag/v2.16.0) · 全部版本见 [Releases](https://github.com/maojiebc/majia-Popskill/releases) · 更新日志在 `docs/release/`
