...
**v0.2 评估项：是否新增 `plugin` 一类 component kind？**（2026-05-15 沉淀）

dotey 2026-05-11 长推（126K views）提出"Agent 版 App Store"概念，特指 Codex / Cursor / Claude Desktop 这些 host app 各自的**插件市场**——比如 Codex Presentations Plugin。这层"插件"跟当前 7 类 component 都不太一样：
...
1. **作为新 kind `plugin`**：跟 Skill / CLI / MCP / Agent 平级，独立 component kind。代价：要改 `AssetComponentKind` enum（Rust + Swift + tests），影响 8 类 schema 稳定性
2. **作为 Skill 的 deployment 模式**：通过 deployment 层的 `wrapper` 或 `configPatch` 策略把 Skill 投射到 host app 的插件接口

**倾向方向 2**，理由：popskill 的 SSOT 不是 host app 的插件市场（那是 SaaS-owned），而是 popskill-owned package。把 host plugin 接口当作 deployment target 更符合 asset control plane 的「source → package → component → deployment → runtime」分层抽象。

**决策窗口**：v0.2 Package 重构周期。v0.1 不动 ComponentKind enum，避免发布延期。在那之前，popskill 通过 deployment 层的 `wrapper` 策略可以临时支撑这类插件的安装/管理（如果有需求）。
