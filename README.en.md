...
[**↓ Download Popskill (3.1 MB, signed + notarized)**](https://github.com/maojiebc/majia-Popskill/releases/latest/download/Popskill-2.21.0.dmg)
...
- **Content-hash updates** — no semver required: per-member SHA-256 against upstream; one clone checks a whole monorepo, only changed members get replaced
- **One-click upstream adds** — new skills that appear upstream show a "+N new" badge: see the names, install them into the store and mount in one click (v2.17)
...
No. 100% local, no analytics, no telemetry, no first-party server. "Local" doesn't mean "never touches the network" — it talks to endpoints tied to features you can see: ① `git clone` / `ls-remote` of the GitHub skill sources **you added**; ② registry.npmjs.org for npm-sourced entries and the CLI patrol (allowlist by default; every global npm package name leaves the machine only if you enable “inspect all” or open the CLI panel); ③ PyPI for allowlisted pipx / uv tools; ④ HTTPS GET of `SKILL.md` for well-known-protocol sources; ⑤ Sparkle checking for app updates. Full endpoint and filesystem disclosure: [SECURITY.md](./SECURITY.md).
...
Current: [v2.21.0](https://github.com/maojiebc/majia-Popskill/releases/tag/v2.21.0) · all versions on [Releases](https://github.com/maojiebc/majia-Popskill/releases) · changelogs in `docs/release/`
